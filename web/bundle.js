    ...
    function(r, p, w) {
      ...
      for (let a of b.servers) {
        ...
        b.partyLink = 0;
                ...
                if(!b.server)
                  {do b.server = b.server[Math.floor(Math.random() * b.servers.length)]; 
                   while(b.server.visible<(ka?1:2))}var z=document.getElementById("serverSelector");  */
        b.server = a;
      }
      let bb = (() => {
          //ID for u know.
          let b = !1,
            ...
            // z = document.getElementById("serverSelector"),
          ...
          g.onmessage = function(c) {
            ...
            switch (c.shift()) {
              ...
              case "c":
                ...
                z.renderv = z.view;
                ia &&
                  (clearInterval(qa), //An Message For Adblockers
                  Y.push({
                    text:
          ...
          g.onclose = function(a) {
            ...
            b.isInGame &&
              ...
              b.died ||
                b.message ||
                (b.message =
                  "Socket closed. If you disconnected, The dev is updating or theres an error"));
            console.warn("WebSocket closed: ", a);
          };
          g.onerror = function(a) {
            console.warn("WebSocket error", a);
            b.message || (b.message = "Socket error. Try Reloading");
            b.isInGame = !1;
          };
          return g;
        };
      ...
      const Sa = (() => {
          ...
          var c = [
            [
              ...
              "Tip: You can play on mobile by just going to real-arrasicoi.glitch.me in your phone!"
            ],
            [
              ...
              "sad: You can cayour own private server with the template in the link on the options menu.",
              "Tip: You can create your own theme with the custom theme maker in the link on the options menu.",
              "Tip: If Arrasicoi is broken you can contact me *marcusicoi9@gmail.com* in the link on options menu.",
              "Tip: Sometimes When you join in ypur phone and you didnt got level 60 because of lag, just die and you will get level 60!",
              "Game Tip: If an spike is faster than you. ram him you you are stronger, if an spike is slower not ram it, the spike will be stronger and youre dead"
            ],
            [
            ...
            ["richi is coping rn", "you're mother"]
          ];
        ...
        servers: [
          {
            ...
            type: "0unk",
            code: "your-server-f",
            at: p.glitch("arras-template-completed")
          },/*
          {
            id: "at",
            type: "1ffa",
            code: "arrasicoi-server-heroku",
            at: p.heroku("arrasicoi-server")
          },*/ {
            id: "at",
            type: "1ffa",
            code: "arrasicoi-server-glitch",
            at: p.glitch("dfqvi2951-eib81eia-oiwo929uwu-developer-client-f")
          }
        ]
          .map((a, e) => ({
            data: a,
